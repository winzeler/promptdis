...
import { PromptClient, NotFoundError, PromptdisError } from "../src/index.js";
...
const prompt = await client.getByName("demo", "myapp", "greeting");
console.log(`  ID:          ${prompt.id}`);
console.log(`  Name:        ${prompt.name}`);
console.log(`  Version:     ${prompt.version}`);
console.log(`  Model:       ${prompt.model?.default ?? "unknown"}`);
console.log(`  Environment: ${prompt.environment}`);
console.log(`  Body:        ${prompt.body.slice(0, 80)}...`);

// 2. Fetch the same prompt by ID
console.log("\n--- Fetch by ID ---");
const samePrompt = await client.get(prompt.id);
...
try {
  ...
  if (e instanceof NotFoundError) {
    ...
    console.log(`  Caught PromptdisError: ${e.message}`);
  }
}
